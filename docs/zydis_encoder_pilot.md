...
## 独立批次 20：重写 `TestBridgeThunkRealPeLoaderException`——只改测试 fixture，不改生产 thunk（2026-07-23）

### 起点

批次 19（提交 `3831388`）push 后，static-gate 与 x64 均 `success`，Win32 job
`failure`：`ctest` 里 `vm_handler_synthesis` 真的 FAILED（不是本机之前那次
并发资源争抢导致的偶发 flake——同一提交在本机单独跑两次这个测试都是干净通过
的，只有 GitHub 的 Win32 runner 上失败），具体断言是
`TestBridgeThunkRealPeLoaderException`（批次 17 新增、批次 18/19 都没有碰过
的 Win32 真实 PE + CreateProcess 异常测试）：故障用例的最终进程退出码是
`0xC0000029`（STATUS_INVALID_DISPOSITION），不是预期的 `0xC0000005`
（EXCEPTION_ACCESS_VIOLATION）。仓库所有者复查这个 fixture，指出真正的根因
和正确的修复方向（不是加大栈缓冲区、也不是把 `0xC0000029` 加进允许结果列表）：

1. `state.gpr[ESP]`（`BuildX86Thunk` 执行被抽取指令前会把真实 ESP 切到这个
   值）此前指向 fixture 自己 `.data` 段里一块普通的 4KB 缓冲区，不是被 TEB
   `StackBase`/`StackLimit` 覆盖、带 guard page 的真实 OS 线程栈。故障发生
   时，`KiUserExceptionDispatcher` 本身要用*当前* ESP 在栈上搭建临时帧再
   逐个调用异常处理链——ESP 指向的不是真实栈，这套机制自身的行为在不同
   Windows 版本/安全配置下不可预期，这正是本机（真实复现
   `EXCEPTION_ACCESS_VIOLATION`）与 GitHub runner（`STATUS_INVALID_DISPOSITION`）
   结果不一致的根因。加大 `.data` 里的假栈缓冲区治标不治本，问题的本质是
   "这根本不是一个真实栈"，不是"栈不够大"。
2. 原本只看最终 `GetExitCodeProcess` 退出码，这本身就是"没有任何处理器时
   Windows 最终如何收尾一个（可能已经不正常的）异常栈环境"的产物，不是故障
   本身的直接证据，会把"故障分发机制本身出了问题"和"故障本身没有按预期
   发生"混为一谈。

### 修复：只改测试 fixture，不改任何生产代码

`packer/transforms/vm_instruction_bridge_builder.cpp`（`BuildX86Thunk`/
`BuildX64Thunk`）本批完全没有改动——问题出在批次 17 这个测试自己构造的
"guest 栈"不是真实栈，不是 thunk 的 ESP 切换逻辑本身有问题。改动全部在
`tests/test_vm_handler_synthesis.cpp` 的 `TestBridgeThunkRealPeLoaderException`
一节：

1. **guest ESP 改用真实线程栈。** harness 机器码从"把编译期算好的 `.data`
   固定偏移写进 `state.gpr[ESP]`"改成运行时执行 `mov eax,esp` 读真实当前
   ESP，`sub eax,0x1000` 留出安全余量（覆盖 harness 自己随后 `push`/`call`
   用掉的 8 字节，且仍在线程默认 1MB 栈预留范围内，需要时由 Windows 的
   guard page 机制自动提交增长，跟任何一次普通、合法的深层调用栈增长完全
   同构），再把这个真实值写回 `state.gpr[ESP]`。不再需要专门在 `.data` 里
   预留一块 4KB 的"假栈"，`kDataSize` 相应简化为只有 state 结构体 + FXSAVE
   镜像。新增 `RealPeMovAbsFromEax` 汇编 helper（`mov [va],eax`，用于把
   运行时才知道的寄存器值写回固定地址——`RealPeMovAbsImm32` 只能写编译期
   常量，做不到这一点）。
2. **故障证据改为 first-chance 异常事件，不再看最终退出码。**
   `CreateProcess` 从普通模式改为 `DEBUG_ONLY_THIS_PROCESS`，本测试自己
   当调试器，`WaitForDebugEvent` 循环捕获 Windows 在搜索任何异常处理器
   *之前*就会通知调试器的原始 `EXCEPTION_DEBUG_EVENT`（`dwFirstChance!=0`）
   ——这是故障本身最原始的证据，不掺杂"没有处理器时 Windows 最终如何收尾"
   这一步的任何变数。新增 `RealPeFaultEvidence`/`RunRealPeExceptionFixtureDebugged`：
   捕获到第一个 first-chance 异常后立即 `GetThreadContext` 读 CPU 现场、
   `ReadProcessMemory` 读回 harness 自己写进 `state.gpr[ESP]` 的真实值，
   再 `TerminateProcess` 结束子进程（后续以哪个 NTSTATUS 退出不再参与任何
   判定）。`TestBridgeThunkRealPeLoaderException` 现在核对：
   - `ExceptionCode == EXCEPTION_ACCESS_VIOLATION`；
   - `ExceptionAddress`/`Eip` 等于 `kRealPeImageBase +
     link.nativeInstructionRVA`（直接用 Builder 自己报告的 RVA，不需要
     调用方重新猜测/重新计算故障地址）；
   - `Ecx == 0`（`MOV EAX,[ECX]` 的地址操作数）；
   - `Edx == stateVA`（hidden=EDX，被抽取指令执行期间 EDX 应持续指向
     state）；
   - `Esp` 与 `ReadProcessMemory` 读回的 `state.gpr[ESP]` 完全一致（即
     thunk 真的把 ESP 切到了 harness 要求的那个真实栈地址，不多不少）；
   - `ExceptionInformation[0]==0`（读操作）、`ExceptionInformation[1]==0`
     （访问地址 NULL）。

   `BuildRealPeExceptionFixture` 的返回类型从裸 `std::vector<uint8_t>`
   改成 `RealPeFixtureBuildResult{peBytes, nativeInstructionRVA, stateVA,
   gpr4VA}`，把 Builder 自己产出的 `nativeInstructionRVA` 和 fixture 自己
   算出的地址一并带给调用方，而不是让测试重新猜测这些值。

### 一个额外发现：Windows 调试协议自身的 loader breakpoint

本机第一次跑新逻辑时，捕获到的"第一个" first-chance 异常 `ExceptionCode`
是 `0x80000003`（`EXCEPTION_BREAKPOINT`），断言失败。定位后确认：任何用
`DEBUG_ONLY_THIS_PROCESS`/`DEBUG_PROCESS` 附加的进程，在真正跑到自己入口点
之前，ntdll 都会先抛一个 loader breakpoint 通知调试器"进程已就绪"——这是
所有 Win32 调试器都必须识别并跳过（`DBG_CONTINUE`）的标准协议事件，不是
harness 自己的故障。修复：`RunRealPeExceptionFixtureDebugged` 新增
`sawLoaderBreakpoint` 标记，遇到的第一个 `EXCEPTION_BREAKPOINT` 直接跳过
（保持 `DBG_CONTINUE`、不计入证据），只把它之后的下一个异常事件当作真正的
故障证据。

### 验证结果（本机实测，2026-07-23）

Win32（`build32`）Release、`/W4 /WX` 全量重新构建，零警告零错误（x64 完全
不受影响——改动全部在 `#if defined(_M_IX86)` 之内，未重新构建 x64）。
`test_vm_handler_synthesis.exe` 单独运行 exit code 0，实际捕获到的现场：
`EIP=0x403062 ECX=0x0 ESP=0x19ef78 EDX=0x402000
ExceptionInformation=[0x0,0x0]`——`ESP=0x19ef78` 是一个典型的真实 Win32
线程栈地址（远高于镜像自身 `0x400000`/`0x402000` 区间），直接证明 guest ESP
现在真的落在 OS 分配的真实线程栈里，不再是 `.data` 里的固定小缓冲区。
不与其他构建并发、单独完整跑一遍 Win32 `ctest --test-dir build32 -C Release
--output-on-failure --timeout 300`：17/18 通过（701.42 秒，
`vm_per_build_similarity_gate` 542.73 秒 PASS，`vm_handler_synthesis` 干净
通过），唯一"失败"仍是 `vm_native_differential` 既有 `TIMEOUT 120`（与
历史批次相同的已知本机限制，未修改该属性）。

### 已知缺口（未变化）

批次 17/18/19 记录的已知缺口（x64 真实故障注入窗口、checked arithmetic
溢出分支、AVX 单机验证）不因本批而消失；本批修的是批次 17 遗留的测试
fixture 设计问题（不是生产 `BuildX86Thunk`/`BuildX64Thunk` 的逻辑问题），
不重新宣称 `BRIDGE_EXTENDED` 100% 完成。本批未在能复现 `STATUS_INVALID_
DISPOSITION` 的 GitHub Win32 runner 环境上直接验证过修复效果（本机从未
复现过这个问题），最终确认要看远端 CI 结果。
