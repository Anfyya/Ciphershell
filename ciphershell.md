...
- 仅函数内 VM CALL 可用；外部/间接 native CALL 在 ABI 与参数恢复可证明前 fail-closed。
- 编译通过与静态检查流程。

### 14.2 当前尚未达到最终架构的部分

- 外部 CALL/native bridge/import bridge 的参数与 ABI 恢复未闭环。
...
- 普通 section/启动字符串加密仍使用未认证滚动/XOR格式并携带可恢复任务密钥。
- CFG flattening 与 Bogus Flow 尚未具备 RIP/CALL、ABI、unwind、CFG 和原语义保持证明。
- 上述未闭环模块默认关闭，显式启用由 CapabilityChecker 拒绝。
- 未由用户完成运行时验证。
