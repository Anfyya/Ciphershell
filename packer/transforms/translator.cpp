...
bool Translator::TranslateCall(const InstructionIR& instruction, BytecodeInstr& output) {
    // 只允许能够静态证明属于当前 VM 控制流范围、且不需要 native ABI 恢复的直接 VM CALL。
    // 任何需要 native ABI / 外部目标 / 间接目标的 CALL 都无法被静态证明，必须翻译失败，
    // 并进入现有 TranslationFailure 体系（保留 RVA、原始字节、原因）。
    // 绝不生成 VM_CALL_NATIVE/VM_CALL_IMPORT/VM_CALL_INDIRECT_* 后假设运行时兜底。
    if (instruction.isIndirectBranch) {
        const auto operands = SemanticOperands(instruction);
        if (operands.size() != 1) {
            return FailInstruction(instruction, "indirect CALL requires one target operand");
        }
        if (IsRegister(operands[0])) {
            return FailInstruction(instruction,
                "indirect register CALL has no statically provable native ABI or VM control-flow proof");
        }
        if (IsMemory(operands[0])) {
            if (operands[0]->memory.isImageAddress &&
                m_config.importThunkRVAs.count(operands[0]->memory.resolvedRVA) != 0) {
                return FailInstruction(instruction,
                    "import CALL does not rewrite the real IAT callsite; "
                    "runtime resolver ABI cannot be statically proven");
            }
            return FailInstruction(instruction,
                "indirect memory CALL has no statically provable native ABI or VM control-flow proof");
        }
        return FailInstruction(instruction, "indirect CALL target must be register or memory");
    }
    if (!instruction.hasBranchTarget) {
        return FailInstruction(instruction, "direct CALL has no target RVA");
    }
    ...
    return FailInstruction(instruction,
        "external native CALL requires native ABI recovery that the VM cannot statically prove");
}
